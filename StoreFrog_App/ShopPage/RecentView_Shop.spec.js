...
//Constants
const newtitle = 'Updated RecentViewed - Shop';
...
test.beforeAll(async ({browser}) => {
    ...
    await RecentViewedProducts(recentViewed_products,newPage,storeURL,storeURL);
...
// 1. Create new Widget
test('Create new RecentViewed widget for Shop page',{tag:'@CreateNewWidget'}, async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'RecentviewShop.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'Recently viewed');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'RecentviewShop.json'), JSON.stringify({ widgetID }));
...
test('Edit Widget title',{tag:'@EditTitle'}, async ()=> {
    ...
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'RecentviewShop.json'))); 
