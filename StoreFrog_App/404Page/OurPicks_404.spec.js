...
//Constants
const newtitle = 'New OurPicks - 404';
...
// 1. Create new Widget
test('Create new OurPicks widget for 404 page',{tag:'@CreateNewWidget'}, async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'Ourpicks404.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'Our pick for you');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'Ourpicks404.json'), JSON.stringify({ widgetID }));
...
test('Edit Widget title', {tag:'@EditTitle'},async ()=> {
    ...
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'Ourpicks404.json'))); 
