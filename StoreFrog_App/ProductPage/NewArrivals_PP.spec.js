...
const newtitle = 'Updated NewArrivals - PP';
...
// 1. Create new Widget
test('Create new NewArrival widget for Product page',{tag:'@CreateNewWidget'}, async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'NewarrivalsPP.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'New arrivals');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'NewarrivalsPP.json'), JSON.stringify({ widgetID }));
...
test('Edit Widget title', {tag:'@EditTitle'},async ()=> {
    ...
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'NewarrivalsPP.json'))); 
