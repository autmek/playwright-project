...
const newtitle = 'Updated HotSeller - Shop';
...
// CreateNewWidget - HotSeller
test('Create new HotSeller widget for Shop page', async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'HotsellerShop.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'Hot selling');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'HotsellerShop.json'), JSON.stringify({ widgetID }));
...
test('Add variable product from widget to cart', async () => {
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'HotsellerShop.json'))); 
