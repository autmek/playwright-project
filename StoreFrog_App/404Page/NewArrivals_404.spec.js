...
// Constants
const newtitle = 'NEW newArrivals - 404';
...
// 1. Create new Widget
test('Create new NewArrivals widget for 404 page',{tag:'@CreateNewWidget'}, async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'Newarrivals404.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
...
test('Edit Widget title',{tag:'@EditTitle'}, async ()=> {
    ...
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'Newarrivals404.json'))); 
