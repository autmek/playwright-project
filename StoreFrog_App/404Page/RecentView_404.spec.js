...
//Constants
const newtitle = 'New RecentViewed - 404';
...
// 1. Create new Widget
test('Create new RecentView widget for 404 page',{tag:'@CreateNewWidget'}, async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'RecentView404.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'Recently viewed');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'RecentView404.json'), JSON.stringify({ widgetID }));
