...
//Constants
const newtitle = 'Updated BestSeller - Collection';
...
// 1. Create new Widget
test('Create new BestSeller widget for Collection page', {tag:'@CreateNewWidget'},async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'BestsellerCP.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'Best sellers');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'BestsellerCP.json'), JSON.stringify({ widgetID }));
...
test('Edit Widget title',{tag:'@EditTitle'}, async ()=> {
    ...
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'BestsellerCP.json'))); 
