...
// FBT function
const{deleteAllotherRecommendation,
...
test('Create new FBT widget for Cart page', {tag:'@CreateNewWidget'},async()=>{
    ...
    fs.writeFileSync(path.resolve(__dirname, 'fbtCart.json'), JSON.stringify({ widgetID }));
...
test.describe('FBT - Discounts', {tag:'@Discounts'},async()=>{
    ...
    //Make sure the discount text is "Buy this bundle at {discount} off" to avoid conflict in the code
