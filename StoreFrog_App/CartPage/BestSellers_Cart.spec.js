...
const newtitle = 'Updated BestSeller - Cart';
...
// 1. Create new Widget
test('Create new BestSeller widget for Cart page',{tag:'@CreateNewWidget'},async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'BestsellerCart.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'Best sellers');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'BestsellerCart.json'), JSON.stringify({ widgetID }));
...
test('Edit Widget title',{tag:'@EditTitle'},async ()=> {
    ...
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'BestsellerCart.json'))); 
