...
const newtitle = 'Custom widget - Shop';
...
// 1. Create new Widget
test('Create new Custom widget (Onsale products) for shop page',{tag:'@CreateNewWidget'}, async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'CustomShop.json'), JSON.stringify({}));
    await CreateNewWidget(page,iframe,appName,pageName,newtitle);
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'CustomShop.json'), JSON.stringify({ widgetID }));
