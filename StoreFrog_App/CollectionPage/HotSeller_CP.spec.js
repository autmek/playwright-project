...
//Constants
const newtitle = 'Updated HotSeller - Collection';
...
// 1. Create new Widget
test('Create new HotSeller widget for Collection page',{tag:'@CreateNewWidget'}, async()=>{
    fs.writeFileSync(path.resolve(__dirname, 'HotsellerCP.json'), JSON.stringify({}));
    await page.waitForLoadState('load');
    await CreateNewWidget(page, iframe, appName,pageName, 'Hot selling');
    widgetID = await FindWidgetID(iframe);
    fs.writeFileSync(path.resolve(__dirname, 'HotsellerCP.json'), JSON.stringify({ widgetID }));
...
test('Edit Widget title', {tag:'@EditTitle'},async ()=> {
    ...
    if(!widgetID){
        const data= JSON.parse(fs.readFileSync(path.resolve(__dirname, 'HotsellerCP.json'))); 
